...
// 1. Definim categoria pentru filtrarea inițială din Supabase
const CATEGORIE_CURENTA = 'rufe'; 
...
async function fetchProducts() {
    ...
    // 2. Extragem subcategoria din URL (?sub=Detergent Lichid)
    const urlParams = new URLSearchParams(window.location.search);
    let subcategorieURL = urlParams.get('sub');

    // 3. Construim interogarea (query)
    let query = window.supaClient
        .from('produse')
        .select('*')
        .eq('categorie', CATEGORIE_CURENTA);

    // 4. Dacă există subcategorie în URL, filtrăm produsele
    if (subcategorieURL) {
        query = query.eq('subcategorie', subcategorieURL.trim());
        
        // Actualizăm subtitlul paginii cu numele filtrului
    ...
    if (!produse || produse.length === 0) {
        container.innerHTML = `
            <div style="grid-column: 1/-1; text-align: center; padding: 2rem;">
                ...
                <a href="rufe.html" style="color: var(--main-blue); text-decoration: underline;">Vezi toate produsele pentru rufe</a>
            </div>`;
        isFetching = false;
        return;
    }
    ...
    // Rulăm logica de scroll pentru rezultatele din Search
    handleSearchNavigation();
}

/**
 * Verifică dacă există un ID în URL (din search) și face scroll la el
 */
...
// Pornim încărcarea la evenimentul DOMContentLoaded
